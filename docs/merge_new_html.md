...
> **Source repo:** https://github.com/mdwikicx/new_html
> **Live PHP tool:** https://mdwikicx.toolforge.org/new_html_1
> **Target repo:** https://github.com/MrIbrahem/publish_py
> **Owner of this plan:** `MrIbrahem`
...
currently listed as a *Code Source* but has **no** Flask endpoint yet).
...
- The `fix_wikitext` cleanup is **distinct** from the existing `refs` blueprint
  (`text_processor.do_changes_to_text_with_settings`), which *fixes references for
  publishing*. The `new_html` cleanup *strips content to prepare it for translation*.
