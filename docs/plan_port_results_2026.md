...
| 2   | `super_function()` switches between TD-API and SQL based on a setting                                     | Always go through SQLAlchemy; the publish_py architecture has already chosen the SQL path. The "TD-API fallback" is irrelevant inside publish_py because publish_py _is_ the SQL backend.                                                                                                                                                                                                                                                                |
