...
def count_translated(lang: str = "All") -> int:
    ...
    query = pages_crud.session.query(func.count(PageRecord.id)).filter(PageRecord.target.isnot(None), PageRecord.target != "")
    if lang and lang.lower() != "all":
        query = query.filter(PageRecord.lang == lang)
...
def list_pages() -> list[PageRecord]:
    """Return all pages."""
    return list(pages_crud.list(order_by=[PageRecord.id.asc()]))
