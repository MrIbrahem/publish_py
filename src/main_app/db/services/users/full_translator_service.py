...
def list_active_full_translators() -> list[FullTranslatorRecord]:
    ...
    return list(full_translator_crud.list(filters={"is_active": 1}, order_by=[FullTranslatorRecord.id.asc()]))
...
def add_or_update_full_translator(user: str, is_active: int = 1) -> FullTranslatorRecord:
    ...
    return full_translator_crud.upsert(keys={"user": user}, is_active=is_active)
