"""Port of ``results_27/Rows/InProcessRowBuilder.php``.

Builds one row dict for the In-process table. Mirrors PHP
``InProcessRowBuilder::build()``, but returns data for the Jinja partial
...
class InProcessRowBuilder:
    ...
    def _translate_html(
        ...
        *,
        ...
        Returns the HTML for the Translate column. When the button is
        disabled, returns an empty string (the column then renders blank).
        """
        if tra_btn != "1":
            return ""
