...
class TestPublishEndpointWithDenyCSRF:
    ...
    def test_cors_validation_still_works(self, csrf_client):
        ...
        with (
            patch("src.app_main.shared.cors.is_allowed_checker.is_allowed") as mock_deny,
...
class TestPublishEndpointWithCSRF2:
    ...
    def mock_is_allowed(self):
        ...
        auto allow all
        """
        with patch("src.app_main.shared.cors.is_allowed") as mocked:
...
class BasePublishTest:
    ...
    @pytest.fixture(autouse=True)
    def mock_token(self):
        with patch("src.app_main.public.routes.publish.routes.get_user_token_by_username") as mock_get_token:
            token = MagicMock()
