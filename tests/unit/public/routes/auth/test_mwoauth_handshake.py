...
class TestGetHandshaker:
    ...
    def test_raises_when_oauth_config_none(self):
        ...
        with patch("src.main_app.services.auth.auth_service.settings") as mock_settings:
            mock_settings.oauth = None

            with pytest.raises(RuntimeError, match="MediaWiki OAuth configuration is incomplete"):
                get_handshaker()

    @patch("src.main_app.services.auth.auth_service.mwoauth")
...
class TestStartLogin:
    ...
    @patch("src.main_app.services.auth.auth_service.get_handshaker")
