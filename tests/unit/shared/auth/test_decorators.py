...
from src.app_main.shared.auth.decorators import (
    oauth_required,
)
...
class TestOAuthRequired:
    ...
    def test_redirects_when_no_user_and_oauth_enabled(self, app, monkeypatch):
        ...
        monkeypatch.setattr("src.app_main.shared.auth.decorators.current_user", lambda: None)
        ...
        monkeypatch.setattr("src.app_main.shared.auth.decorators.settings", mock_settings)
    ...
    def test_allows_access_when_user_present(self, app, monkeypatch):
        ...
        mock_user = MagicMock()
        monkeypatch.setattr("src.app_main.shared.auth.decorators.current_user", lambda: mock_user)
        # Create a mock Settings object with OAuth enabled
        mock_settings = MagicMock()
        mock_settings.oauth = OAuthConfig(
            mw_uri="https://test.com",
