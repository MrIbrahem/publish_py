...
import pytest
from src.app_main.shared.core.cors.publish_secret_checks import (
...
class TestGetPublishSecretCode:
    ...
    def test_returns_secret_from_settings(self, monkeypatch):
        ...
        monkeypatch.setattr("src.app_main.shared.cors.publish_secret_checks.settings", mock_settings)
...
class TestCheckPublishSecretCode:
    ...
    def test_returns_none_when_no_secret_configured(self, app, monkeypatch):
        ...
        monkeypatch.setattr("src.app_main.shared.cors.publish_secret_checks._get_publish_secret_code", lambda: "")
    ...
    def test_returns_none_when_no_header_present(self, app, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.app_main.shared.cors.publish_secret_checks._get_publish_secret_code", lambda: "expected_secret"
        )
    ...
    def test_returns_host_when_header_matches(self, app, monkeypatch):
        ...
        monkeypatch.setattr(
            "src.app_main.shared.cors.publish_secret_checks._get_publish_secret_code", lambda: "correct_secret"
        )
    ...
    def test_uses_timing_safe_comparison(self, app, monkeypatch):
        ...
        monkeypatch.setattr("src.app_main.shared.cors.publish_secret_checks._get_publish_secret_code", lambda: "secret")

        with patch("src.app_main.shared.cors.publish_secret_checks.hmac.compare_digest") as mock_compare:
