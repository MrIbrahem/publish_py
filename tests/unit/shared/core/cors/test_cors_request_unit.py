...
def mock_request(mocker):
    ...
    mocker.patch("src.app_main.shared.core.cors.request", mock_req)

    mock_app = MagicMock()
    mock_app.config = {"CORS_DISABLED": False}
    mocker.patch("src.app_main.shared.core.cors.is_allowed_checker.current_app", mock_app)

    mock_settings = MagicMock()
    mock_settings.cors.allowed_domains = ["trusted.com", "api.partner.net"]
    mocker.patch("src.app_main.shared.core.cors.is_allowed_checker.settings", mock_settings)
