...
from src.new_app.shared.cors import check_cors
...
class TestCheckCorsDecoratedWithCorsDisabled:
    def test_basics_empty_headers(self, mocker, app):
        ...
        mocker.patch("src.new_app.shared.cors._load_request", return_value=response)
...
class TestCheckCorsDecoratedWithCorsEnabled:
    def test_no_allowed_domains(self, mocker, app):
        ...
        mocker.patch("src.new_app.shared.cors._load_request", return_value=response)
        mocker.patch("src.new_app.shared.cors.is_allowed_checker._get_allowed_domains", return_value=[])
    ...
    def test_basics_200(self, mocker, app):
        ...
        app.config["CORS_DISABLED"] = False
        mocker.patch("src.new_app.shared.cors._load_request", return_value=response)
        mocker.patch("src.new_app.shared.cors.is_allowed_checker._get_allowed_domains", return_value=["z.com"])
    ...
    def test_basics_same_host_url(self, mocker, app):
        ...
        mocker.patch("src.new_app.shared.cors._load_request", return_value=response)
        mocker.patch(
            "src.new_app.shared.cors.is_allowed_checker._get_allowed_domains", return_value=["z.com.net", "z.wikipedia.org"]
        )
