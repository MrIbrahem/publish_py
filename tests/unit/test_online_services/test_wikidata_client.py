...
class TestGetQidForMdtitle:
    ...
    def test_returns_qid_when_found(self):
        ...
        with patch("src.app_main.online_services.wikidata_client.get_db") as mock_get_db:
            mock_db = MagicMock()
            mock_db.fetch_query_safe.return_value = [{"qid": "Q12345"}]
            mock_get_db.return_value = mock_db

            from src.app_main.online_services.wikidata_client import get_qid_for_mdtitle
...
class TestLinkToWikidata:
    ...
    def test_returns_success_on_successful_link(self):
        ...
        with (
            patch("src.app_main.online_services.wikidata_client.get_qid_for_mdtitle") as mock_qid,
            patch("src.app_main.online_services.wikidata_client._link_it") as mock_link,
        ):
            mock_qid.return_value = "Q12345"
            mock_link.return_value = {"success": True}

            from src.app_main.online_services.wikidata_client import link_to_wikidata
    ...
    def test_returns_error_when_no_credentials(self):
        ...
        with patch("src.app_main.online_services.wikidata_client.get_qid_for_mdtitle") as mock_qid:
            mock_qid.return_value = "Q12345"
