...
from src.main_app.services.core.cors import check_cors
...
class TestCheckCorsDecoratedWithCorsDisabled:
    def test_basics_empty_headers(self, mocker, mock_app):
        ...
        mocker.patch("src.main_app.services.core.cors._load_request", return_value=response)
...
class TestCheckCorsDecoratedWithCorsEnabled:
    def test_no_allowed_domains(self, mocker, mock_app):
        ...
        mocker.patch("src.main_app.services.core.cors._load_request", return_value=response)
        mocker.patch("src.main_app.services.core.cors.is_allowed_checker._get_allowed_domains", return_value=[])
    ...
    def test_basics_200(self, mocker, mock_app):
        ...
        mock_app.config["CORS_DISABLED"] = False
        mocker.patch("src.main_app.services.core.cors._load_request", return_value=response)
        mocker.patch("src.main_app.services.core.cors.is_allowed_checker._get_allowed_domains", return_value=["z.com"])
    ...
    def test_basics_same_host_url(self, mocker, mock_app):
        ...
        mocker.patch("src.main_app.services.core.cors._load_request", return_value=response)
        mocker.patch(
            "src.main_app.services.core.cors.is_allowed_checker._get_allowed_domains",
